...
| Multi-agent orchestration | ✅ | `Workflow` ships sequential, parallel, reflection, planner, supervisor (ADR-0007), critic, consensus (ADR-0011), debate, map-reduce (ADR-0012), hierarchical (ADR-0013), graph (ADR-0019), tree-of-thoughts (ADR-0018), and reflexion (ADR-0036) on the native orchestrator; the first twelve also run on the langgraph backend (ADR-0016, ADR-0028, ADR-0029, ADR-0032, ADR-0033, ADR-0034, ADR-0035) -- reflexion is native-only so far — see the Multi-Agent System table below for the rest |
...
| Reflexion | ✅ | `native` orchestrator only — single agent (`steps[0]`) attempts the task, is scored by a pluggable `evaluator=` (falls back to the same agent judging itself via structured output), and reflects on failure before the next of up to `max_trials` independent trials; ADR-0036. LangGraph parity not yet built |
