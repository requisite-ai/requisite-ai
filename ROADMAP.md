...
| Planner strategy | ✅ — `native` and `langgraph` orchestrators; see [ADR-0007](docs/adr/0007-multi-agent-orchestration-strategies.md), [ADR-0035](docs/adr/0035-langgraph-planner-strategy.md) -- at the time this closed every open native-vs-langgraph parity gap for the twelve strategies that existed then; `reflexion` (below) reopened and has since re-closed it |
...
| Tree-of-thoughts strategy | ✅ — `native` and `langgraph` orchestrators; evaluator (`steps[0]`) scores candidates thinkers (`steps[1:]`) generate, branching and pruning a search tree via `breadth`/`beam_width`/`max_depth`; see [ADR-0018](docs/adr/0018-tree-of-thoughts-strategy.md), [ADR-0034](docs/adr/0034-langgraph-tree-of-thoughts-strategy.md) |
| Reflexion strategy (Actor/Evaluator/Self-Reflection: re-attempt the task from scratch across independent trials, informed by accumulated verbal reflection on prior failures) | ✅ — `native` and `langgraph` orchestrators; a pluggable `evaluator=` scores each attempt (falls back to the same agent judging itself via structured output), up to `max_trials`; see [ADR-0036](docs/adr/0036-reflexion-strategy.md), [ADR-0037](docs/adr/0037-langgraph-reflexion-strategy.md) -- closes the last open native-vs-langgraph parity gap, all thirteen multi-agent strategies now run on both backends |
