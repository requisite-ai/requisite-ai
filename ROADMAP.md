...
| Reflexion strategy (Actor/Evaluator/Self-Reflection: re-attempt the task from scratch across independent trials, informed by accumulated verbal reflection on prior failures) | ✅ — `native` orchestrator only; a pluggable `evaluator=` scores each attempt (falls back to the same agent judging itself via structured output), up to `max_trials`; see [ADR-0036](docs/adr/0036-reflexion-strategy.md). `langgraph` parity not yet built |
